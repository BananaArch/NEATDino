...
class Obstacles:
    ...
    def update_obstacles(self):
        ...
        for obstacle in self.obstacles:
            if not obstacle.passed and obstacle.x < self.x_threshold:
                ...
                self.x_threshold = random.uniform(.2 * SCREEN_WIDTH, .4 * SCREEN_WIDTH)
    ...
    def next_obstacle(self, dino):
        ...
        return next(obstacle for obstacle in self.obstacles if
                    obstacle.x + obstacle.img.get_width() > dino.x + dino.img.get_width())